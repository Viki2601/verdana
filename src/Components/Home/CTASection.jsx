...
export default function CTASection() {
    ...
    return (
        <section ref={sectionRef} className="relative overflow-hidden bg-[#040a06] py-12">
            <div className="pointer-events-none absolute inset-0 transition-all duration-700 ease-out" style={{background: `radial-gradient(700px circle at ${mousePos.x * 100}% ${mousePos.y * 100}%, rgba(26,92,53,0.09) 0%, transparent 65%)`,}}/>
            ...
            {[
                ...
                <span key={i} className="pointer-events-none absolute select-none" style={{top: s.top, left: s.left, right: s.right, fontSize: s.size, transform: `rotate(${s.rotate})`, opacity: s.opacity,}}>
                    🌿
                </span>
            ))}
            ...
            <div className="relative mx-auto max-w-[680px] px-6 text-center">
                ...
                <h2 className="mb-5 font-serif text-[2.8rem] sm:text-[3.6rem] font-light leading-[1.1] tracking-[-0.01em] text-[#f4efe6]">
                    The Wild Has 
                    <em className="not-italic" style={{background: 'linear-gradient(135deg, #c9a84c 0%, #f0d080 45%, #c9a84c 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', fontStyle: 'italic', }}>
                        {" "}Much to Whisper
                    </em>
                </h2>

                <p className="mx-auto mb-10 max-w-[66ch] text-[0.85rem] leading-[1.9] text-[rgba(192,212,184,.5)]">
                    Join 12,000+ nature lovers who receive curated retreat recommendations, seasonal offers and trail stories — no spam, ever.
                </p>

                {!sent ? (
                    <div className="mx-auto flex max-w-[500px] flex-col gap-3 sm:flex-row">
                        <div className="relative flex-1 transition-all duration-200" style={{filter: focused ? 'drop-shadow(0 0 14px rgba(201,168,76,0.12))' : 'none',}}>
                            <input
                                type="email"
                        ...
                        <button type="button" onClick={() => email && setSent(true)} className="group relative shrink-0 overflow-hidden rounded-xl bg-[#c9a84c] px-7 py-4 text-[0.82rem] font-semibold tracking-[0.04em] text-[#040a06] transition-all duration-200 hover:bg-[#d9bc6a] active:scale-[0.97]">
                ...
                <div className="mt-14 flex flex-wrap justify-center gap-px overflow-hidden rounded-2xl border border-[rgba(127,176,105,.1)]">
                    {[
                        ...
                        <div key={label} className="flex flex-1 min-w-[110px] flex-col items-center gap-1 bg-[rgba(255,255,255,.025)] px-6 py-5">

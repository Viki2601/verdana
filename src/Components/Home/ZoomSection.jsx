...
// Background: misty forest scene (slower zoom)
const BG =
    'https://images.unsplash.com/photo-1448375240586-882707db888b?w=2400&q=90&auto=format&fit=crop';

// Overlay: dense dark forest canopy (faster zoom — flies toward viewer)
// Dark edges + natural center opening = ideal for the masking effect
const OVERLAY =
...
export default function ZoomSection() {
    ...
    useEffect(() => {
        const onScroll = () => {
            ...
            rafRef.current = requestAnimationFrame(() => {
                ...
                setProgress(p); // only used for text opacity — lightweight
                ...
                if (overlayRef.current) {
                    const s = 1 + p * 1.0;          // scale 1 → 2
                    const z = p * 280;              // translateZ 0 → 280px
                    overlayRef.current.style.transform =
                        `scale(${s}) translateZ(${z}px)`;
                }
            });
        };
    ...
    const textOp = progress < 0.1
        ? 0.15 + progress * 0.5              // barely peeking: 0.15 → 0.2
        : progress < 0.55
            ? 0.2 + ((progress - 0.1) / 0.45) * 0.8   // 0.2 → 1.0
            : 1;

    // Subtle upward drift as text reveals
    const textY = progress < 0.55
        ? 20 - (progress / 0.55) * 20
        : 0;

    return (
        /* 300vh outer = 200vh of scroll fuel */
        <div ref={wrapRef} style={{ position: 'relative', height: '300vh' }}>

            {/* Top blend — joins previous section */}
            <div style={{
                position: 'absolute', top: 0, left: 0, right: 0,
                height: 140, zIndex: 30, pointerEvents: 'none',
                background: 'linear-gradient(to bottom, #060e09, transparent)',
            }} />

            {/* ── Sticky pin: 100vh viewport ──────────────────────── */}
            <div style={{
                ...
                <div style={{
                    ...
                    <p style={{
                        ...
                        fontSize: 'clamp(0.54rem, 0.9vw, 0.65rem)',
                        fontWeight: 400,
                        letterSpacing: '0.55em',
                        textTransform: 'uppercase',
                        color: 'rgba(180,220,185,0.75)',
                        margin: '0 0 1.5rem',
                    }}>✦ Step Into the Wilderness</p>

                    {/* Main heading — large Cormorant */}
                    <h2 style={{
                        ...
                        color: '#f4efe6',
                        margin: '0 0 1rem',
                        textShadow: '0 4px 60px rgba(6,14,9,0.9), 0 0 120px rgba(6,14,9,0.7)',
                    }}>
                        Into the<br />
                        Heart of<br />
                    ...
                    <div style={{
                        ...
                        gap: '0.9rem', margin: '1.8rem 0 1.6rem',
                    }}>
                    ...
                    <div style={{
                        ...
                        <a href="#spots" style={{
                            ...
                            padding: '0.9rem 2.3rem', textDecoration: 'none',
                ...
                {/* ── [3] FOREST OVERLAY — the hero zoom layer ─────────
            perspective on the CONTAINER (not the image) is critical.
            This matches: .image-container { perspective: 500px }
            The image inside gets scale + translateZ.

            mask-image creates the "clearing" composition:
            • Center zone: mask = 0 (overlay invisible) → text shows through
            • Edge zone  : mask = 1 (overlay fully opaque) → dark trees cover text
            As scale 1→2, the opaque edge zone travels off-screen.
        ── */}
                <div style={{
                    ...
                    <img
                        ...
                        style={{
                            ...
                            transformOrigin: 'center center',
                            willChange: 'transform',
                            /* ── The mask: transparent center, opaque edges ──
                               This is what makes the text peek through initially
                               and fully reveal as edges zoom off-screen.         */
                ...
                <div style={{
                    ...
                    background:
                        'radial-gradient(ellipse 80% 75% at 50% 50%, transparent 35%, rgba(6,14,9,0.5) 100%)',
                    pointerEvents: 'none',
                }} />
                ...
                <div style={{
                    ...
                    <svg width="14" height="22" viewBox="0 0 14 22" fill="none"
                        style={{ animation: 'zNudge 2s ease-in-out infinite' }}>
                        <rect x="5.5" y="0.5" width="3" height="13" rx="1.5"
                            stroke="rgba(244,239,230,0.28)" fill="none" />
                        <circle cx="7" cy="4.5" r="1.4" fill="rgba(244,239,230,0.4)">
                            <animate attributeName="cy" values="4.5;9.5;4.5" dur="2s" repeatCount="indefinite" />
                        </circle>
                        <path d="M3 16l4 5 4-5" stroke="rgba(244,239,230,0.22)" fill="none" />
                    </svg>
                </div>
                ...
                <div style={{
                    ...
                    height: 2, background: 'rgba(180,220,185,0.08)', zIndex: 20,
                }}>
                    <div style={{
                        height: '100%',
                        width: `${progress * 100}%`,
                        background: 'linear-gradient(90deg, rgba(180,220,185,0.4), rgba(244,239,230,0.6))',
                ...
                <div style={{
                    position: 'absolute', bottom: 0, left: 0, right: 0,
                    height: 160, zIndex: 15, pointerEvents: 'none',
